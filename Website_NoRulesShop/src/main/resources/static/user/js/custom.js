...
$(document).ready(function () {
    ...
    if ($class.hasClass('category') || $class.hasClass('off-canvas-left') || $class.hasClass('off-canvas-right')) {
        $("#slider-range").slider({
            ...
            // }
            slide: function (event, ui) {
                var formattedValues = formatCurrency(ui.values[0]) + " - " + formatCurrency(ui.values[1]);
                $("#amount").val(formattedValues);
            }
        });
        // $("#amount").val($("#slider-range").slider("values", 0) + " đ - " +
        //     $("#slider-range").slider("values", 1) + " đ");
        var initialFormattedValues = formatCurrency($("#slider-range").slider("values", 0)) + " - " +
            formatCurrency($("#slider-range").slider("values", 1));
        $("#amount").val(initialFormattedValues);
    }
